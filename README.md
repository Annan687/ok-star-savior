...
中斷後再按「開始日課」，會跳過上一輪已處理的項目，從需檢查及尚未執行的項目接續；重開 OKSS 後也保留進度。續跑限電腦本機同一日期、相同勾選與刷關設定。要重跑全部勾選項目，請按「重新開始」。整輪完成後、日期或設定改變後會開始新一輪；計劃任務啟動仍照該排程完整執行，不套用首頁續跑紀錄。「跟隨日課設定」排程中斷後，可按首頁「開始日課」接續同日、同設定的進度；自訂排程保持獨立。
