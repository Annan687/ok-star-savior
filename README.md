...
自動化流程可能因遊戲更新、UI 變更、帳號解鎖進度、網路延遲、
解析度、Windows 環境或其他程式干擾而失效。
...
目前專案仍在測試及安裝器驗證階段；如果 Releases 頁面沒有版本，
代表目前尚未正式發布安裝檔。
...
已包含主要執行環境，一般使用者**不需要另外安裝 Python**。
...
這些資料預設保存在本機，**不會由 OKSS 自動上傳**。

> 回報問題前請確認 log、截圖或其他附件中沒有 UID、
> 帳號資訊、角色名稱或其他不希望公開的內容。
...
如果仍然失敗，可以重新啟動 Star Savior 與 OKSS 後再試。
...
請查看 `logs` 最後的錯誤紀錄，以及 `screenshots`
是否保存了當時畫面。
...
完整版本可能包含 Python 執行環境、自動化相關套件及安裝器。

部分防毒軟體可能因打包程式、自動化輸入、Python 或更新機制產生警告。
...
Online Installer 首次執行需要下載 Python 及相關套件。
...
> **原始碼可用不代表安裝檔已經正式發布。**
>
> 正式版本請以 GitHub Releases 實際提供的內容為準。
...
OKSS 的 Python 相依套件中包含 PySide6-Fluent-Widgets。

該專案對非商業用途採用 GPLv3 授權，
商業使用另有其上游授權要求。
