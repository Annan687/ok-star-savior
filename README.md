...
v0.2.3 介面改為分類展開／折疊：勾選體力刷關、限時據點或激戰委託後，關卡設定會出現在該項下方。取消勾選會隱藏設定並保留原選擇；折疊分類不會取消勾選。活動襲擊與活動任務下方的活動名稱同步，完成後關閉則放在「執行設定」。
...
3. 到左側「計劃任務」建立任務，選擇 Star Savior 日課、每天及執行時間。
4. 若要跑完關閉，勾選排程中的「完成後退出（-e）」。首頁的「完成後關閉遊戲與 OKSS」也能控制手動執行；兩者任一啟用便會在成功結束後關閉。
...
清單有獨立「活動」區，四項可各自勾選。灰色研究的襲擊與每日／點數／特殊任務分開選擇；活動名稱請設為「灰色研究」。只領獎時只勾「活動任務」，不會掃蕩。
...
`v0.2.2` 已公開為**測試版**。以下分別列出日課流程、安裝包與啟動器的驗證結果。

| 範圍 | 已完成的驗證 |
| --- | --- |
| 日課流程 | 既有流程有單一帳號 1600×900 跨日分段續跑紀錄；v0.2.3 通過 219 項離線測試與 21 個子測試，新活動 MAX 分支及案件檔案整合仍待實機驗證。[修正內容](CHANGELOG.md) |
